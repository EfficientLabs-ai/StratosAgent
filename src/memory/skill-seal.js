/**
 * skill-seal.js — the VERIFICATION GATE for federated skill-sync.
 *
 * WHY THIS EXISTS (honest gap it closes): when skill blocks are replicated between nodes, each carries a
 * `signatureSeal` field — but a seal is worthless unless something verifies it before the blocks are
 * returned, or a node could ingest and run a skill from an unauthenticated peer. This module is the gate
 * that MUST wrap ingest: a remote skill is trusted only if its seal verifies under a PINNED origin key.
 *
